...
type WirePath map[int]map[int]int

// toPath will create a wirePath with all the positions the wire has taken
func toPath(r []string) (wirePath WirePath) {

	// Setting initial position
	wirePath = make(map[int]map[int]int, 0)
	wirePath[0] = make(map[int]int, 0)
	wirePath[0][0] = 0

	var x, y, totalSteps int
	for _, v := range r {
		axis := v[:1]
		...
		for i := 0; i < steps; i++ {
			...
			if wirePath[x] == nil {
				wirePath[x] = map[int]int{}
			}
			if wirePath[x][y] == 0 {
				wirePath[x][y] = totalSteps
			}
		}
	}
...
func findShortestStep(w1 WirePath, w2 WirePath) (shortest int) {
	for x, v := range w1 {
		for y, s1 := range v {
			if w2[x][y] == 0 {
				continue
			}
			if s1+w2[x][y] < shortest || shortest == 0 {
				shortest = s1 + w2[x][y]
			}
		}
	}
...
func findShortestDistance(w1 WirePath, w2 WirePath) (shortest int) {
	for x, v := range w1 {
		for y := range v {
			if w2[x][y] == 0 {
				continue
...
func main() {
	...
	awnser1 := findShortestDistance(wirePath1, wirePath2)
	awnser2 := findShortestStep(wirePath1, wirePath2)
