...
// WirePath is the exact path the wire took
type WirePath []position

// toPath will create a wirePath with all the positions the wire has taken
func toPath(r []string) (wirePath WirePath) {
	// This is the starting position
	wirePath = append(wirePath, position{x: 0, y: 0, steps: 0})
	...
	for _, v := range r {
		...
		var pos position
		pos.x = wirePath[len(wirePath)-1].x
		pos.y = wirePath[len(wirePath)-1].y
		pos.steps = wirePath[len(wirePath)-1].steps

		var pathTaken WirePath
		switch axis {
		case "R":
			// X-axis
			pathTaken = takeSteps(pos, X, steps, false)
		case "L":
			// X-axis
			pathTaken = takeSteps(pos, X, steps, true)
		case "U":
			// Y-axis
			pathTaken = takeSteps(pos, Y, steps, false)
		case "D":
			// Y-axis
			pathTaken = takeSteps(pos, Y, steps, true)
		}

		wirePath = append(wirePath, pathTaken...)
	}
	return
}

// takeSteps will create a wirePath based on the given route and steps
func takeSteps(startPos position, axis Axis, steps int, negative bool) (route WirePath) {
	route = append(route, position{x: startPos.x, y: startPos.y, steps: startPos.steps})
	for i := 0; i < steps; i++ {
		var pos position
		if axis == X {
			if negative {
				pos.x = route[len(route)-1].x - 1
				pos.y = route[len(route)-1].y
			} else {
				pos.x = route[len(route)-1].x + 1
				pos.y = route[len(route)-1].y
			}
		...
		if axis == Y {
			if negative {
				pos.x = route[len(route)-1].x
				pos.y = route[len(route)-1].y - 1
			} else {
				pos.x = route[len(route)-1].x
				pos.y = route[len(route)-1].y + 1
			}
		}
		pos.steps = route[len(route)-1].steps + 1
...
func main() {
	...
	log.Println("Pathing wire1")
	wirePath1 := toPath(strToArray(wire1))

	log.Println("Pathing wire2")
	wirePath2 := toPath(strToArray(wire2))

	log.Println("Starting comparison")
	var crossings []crossing
	for _, v1 := range wirePath1 {
		for _, v2 := range wirePath2 {
			if v1.x == v2.x && v1.y == v2.y && v1.x != 0 && v1.y != 0 {
				cros := crossing{
					pos:      v1,
					totSteps: v1.steps + v2.steps,
				}
				crossings = append(crossings, cros)
			}
		}
	}

	log.Println("Finding crossings")
	var awnser1, awnser2 float64
	for _, v := range crossings {
		distance := math.Abs(float64(v.pos.x)) + math.Abs(float64(v.pos.y))
		if awnser1 == 0 || distance < awnser1 {
			awnser1 = distance
		}

		if awnser2 == 0 || v.totSteps < awnser2 {
			awnser2 = v.totSteps
		}
	}
